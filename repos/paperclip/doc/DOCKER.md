...
services:
  paperclip:
    environment:
      ...
      RESEND_FROM_EMAIL: auth@desk.koker.net
...
Email OTP registration uses Resend when `RESEND_API_KEY` is set. For shared or public deployments, `RESEND_FROM_EMAIL` must be on a verified Resend domain; the default `onboarding@resend.dev` sender is test-only and cannot deliver verification codes to arbitrary users.
