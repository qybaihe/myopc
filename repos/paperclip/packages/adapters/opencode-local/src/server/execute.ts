...
function buildSafeRuntimeBaseEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    ...
    if (key.startsWith("LC_") || safeKeys.has(key)) env[key] = value;
  }
  return env;
}

function readOpenCodeSandboxContext(context: Record<string, unknown>) {
  const sandbox = parseObject(context.paperclipSandbox);
  const readonlyToolchains = parseObject(sandbox.readonlyToolchains);
  const opencodeToolchain = parseObject(readonlyToolchains.opencode);
  const env = readStringRecord(sandbox.env);
  return {
    homeDir: asString(sandbox.homeDir, ""),
    workspaceDir: asString(sandbox.workspaceDir, ""),
    tmpDir: asString(sandbox.tmpDir, ""),
    env,
    opencodeToolchain: {
      binDir: asString(opencodeToolchain.binDir, ""),
      command: asString(opencodeToolchain.command, ""),
    },
  };
}

function claudeSkillsHome(homeDir?: string | null): string {
  const root = homeDir && homeDir.trim().length > 0 ? homeDir.trim() : os.homedir();
...
async function buildOpenCodeSkillsDir(config: Record<string, unknown>, tmpDir?: string): Promise<string> {
  const root = tmpDir && tmpDir.trim().length > 0 ? tmpDir.trim() : os.tmpdir();
  await fs.mkdir(root, { recursive: true });
  const tmp = await fs.mkdtemp(path.join(root, "paperclip-opencode-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const sandboxContext = readOpenCodeSandboxContext(context);
  ...
  const env: Record<string, string> = {
    ...(Object.keys(sandboxContext.env).length > 0 ? sandboxContext.env : buildSafeRuntimeBaseEnv()),
    ...buildPaperclipEnv(agent),
  };
  if (sandboxContext.homeDir) {
    env.HOME = sandboxContext.homeDir;
    env.XDG_CONFIG_HOME = env.XDG_CONFIG_HOME || path.join(sandboxContext.homeDir, ".config");
    env.XDG_CACHE_HOME = env.XDG_CACHE_HOME || path.join(sandboxContext.homeDir, ".cache");
  }
  if (sandboxContext.tmpDir) {
    env.TMPDIR = sandboxContext.tmpDir;
    env.TMP = sandboxContext.tmpDir;
    env.TEMP = sandboxContext.tmpDir;
  }
  if (sandboxContext.opencodeToolchain.binDir) {
    env.PATH = [sandboxContext.opencodeToolchain.binDir, env.PATH ?? env.Path]
  ...
  refreshPaperclipWorkspaceEnvForExecution({
    env,
    envConfig,
    workspaceCwd: sandboxContext.workspaceDir || effectiveWorkspaceCwd,
    workspaceSource,
    workspaceId,
    workspaceRepoUrl,
    workspaceRepoRef,
  ...
  try {
    const runtimeEnv = Object.fromEntries(
      Object.entries(ensurePathInEnv({ ...preparedRuntimeConfig.env })).filter(
        (entry): entry is [string, string] => typeof entry[1] === "string",
      ),
    );
    ...
    if (executionTarget?.kind === "remote") {
      localSkillsDir = await buildOpenCodeSkillsDir(config, preparedRuntimeConfig.env.TMPDIR);
      await onLog(
        "stdout",
