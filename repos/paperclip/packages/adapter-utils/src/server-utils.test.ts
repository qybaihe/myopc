...
describe("runChildProcess", () => {
  it("does not inherit host-side service secrets", async () => {
    ...
    try {
      const result = await runChildProcess(
        ...
        [
          "-e",
          [
            "const keys = ['DATABASE_URL', 'RESEND_API_KEY', 'OPENAI_API_KEY', 'EXPLICIT_SECRET'];",
            "process.stdout.write(JSON.stringify(Object.fromEntries(keys.map((key) => [key, process.env[key] ?? null]))));",
          ].join(" "),
        ],
        {
          cwd: process.cwd(),
          env: { EXPLICIT_SECRET: "allowed-explicit-secret" },
          timeoutSec: 5,
          graceSec: 1,
          onLog: async () => {},
        },
      ...
      expect(JSON.parse(result.stdout)).toEqual({
        ...
        RESEND_API_KEY: null,
        OPENAI_API_KEY: null,
        EXPLICIT_SECRET: "allowed-explicit-secret",
      });
    } finally {
