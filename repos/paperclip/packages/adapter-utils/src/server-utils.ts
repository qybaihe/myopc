...
const SENSITIVE_INHERITED_ENV_KEYS = new Set([
  "DATABASE_URL",
  "RESEND_API_KEY",
  "BETTER_AUTH_SECRET",
...
const SENSITIVE_INHERITED_ENV_KEY_RE = /(?:^|_)(?:API[_-]?KEY|SECRET|TOKEN|PASSWORD|PASSWD|AUTHORIZATION|COOKIE|PRIVATE[_-]?KEY|WEBHOOK)(?:$|_)/i;
...
function isSensitiveInheritedEnvKey(key: string): boolean {
  const normalized = key.toUpperCase();
  return SENSITIVE_INHERITED_ENV_KEYS.has(normalized) || SENSITIVE_INHERITED_ENV_KEY_RE.test(key);
}
...
export function sanitizeInheritedPaperclipEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    ...
    if (isSensitiveInheritedEnvKey(key)) {
      delete env[key];
    }
  }
  return env;
