...
import { browserVisibleProjectServiceUrl } from "./utils";

type ProjectWorkspaceLike = Pick<Project, "workspaces" | "primaryWorkspace">;
type ProjectRouteLike = Pick<Project, "id" | "name"> & { urlKey?: string | null };
...
function runtimeServiceSummary(
  ...
  return {
    serviceCount,
    runningServiceCount,
    primaryServiceUrl: project ? browserVisibleProjectServiceUrl(primaryService?.url ?? null, project) : primaryService?.url ?? null,
    primaryServiceUrlRunning: primaryService?.status === "running",
  };
}

export function buildProjectWorkspaceSummaries(input: {
  project: ProjectWorkspaceLike & Partial<ProjectRouteLike>;
  issues: Issue[];
  executionWorkspaces: ExecutionWorkspace[];
}): ProjectWorkspaceSummary[] {
  const projectRoute = input.project.id && input.project.name
    ? { id: input.project.id, urlKey: input.project.urlKey ?? null, name: input.project.name }
    : null;
  ...
  for (const issue of input.issues) {
    if (issue.executionWorkspaceId) {
      ...
      const runtimeSummary = runtimeServiceSummary(executionWorkspace.runtimeServices, projectRoute ?? undefined);
    ...
    const runtimeSummary = runtimeServiceSummary(projectWorkspace.runtimeServices, projectRoute ?? undefined);
