...
describe("buildProjectWorkspaceSummaries", () => {
  ...
  const project = {
    ...
    primaryWorkspace,
  } satisfies Pick<Project, "id" | "urlKey" | "name" | "workspaces" | "primaryWorkspace">;
  ...
  it("rewrites loopback service urls to MyOPC app subdomains on hosted MyOPC", () => {
    ...
    expect(summaries[0]).toMatchObject({
      key: "execution:exec-live",
      primaryServiceUrl: "https://demo-shop.apps.myopc.me",
      primaryServiceUrlRunning: true,
    });
