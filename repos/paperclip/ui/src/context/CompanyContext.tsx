...
export function CompanyProvider({ children }: { children: ReactNode }) {
  ...
  // Some invite-specific flows used to cache a raw Company[] under the shared
  // companies key. Be defensive so a stale mixed-shape cache never takes down
  // the shell while the invite flow refreshes.
  const normalizedCompaniesResult = Array.isArray(companiesResult)
    ? { companies: companiesResult, unauthorized: false }
    : {
        companies: Array.isArray(companiesResult.companies)
          ? companiesResult.companies
          : [],
        unauthorized: Boolean(companiesResult.unauthorized),
      };
