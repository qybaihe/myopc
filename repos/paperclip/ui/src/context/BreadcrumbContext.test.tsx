...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before MyOPC", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • MyOPC",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • MyOPC");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • MyOPC");
    expect(buildDocumentTitle([], null)).toBe("MyOPC");
  });
});
