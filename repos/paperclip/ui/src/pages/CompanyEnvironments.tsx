...
function driverLabel(
  ...
  switch (driver) {
    ...
    case "local":
    default:
      return driver === "local" ? t("Local") : t(driver);
  }
}
...
export function CompanyEnvironments() {
  ...
  const environmentMutation = useMutation({
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId ? t("Environment updated") : t("Environment created"),
        body: t("{{name}} is ready.", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("Failed to save environment"),
        body: error instanceof Error ? error.message : t("Environment save failed."),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok ? t("Environment probe passed") : t("Environment probe failed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      setProbeResults((current) => ({
        ...
        [environmentId]: {
          ...
          summary: error instanceof Error ? error.message : t("Environment probe failed."),
          details: null,
        },
      }));
      pushToast({
        title: t("Environment probe failed"),
        body: error instanceof Error ? error.message : t("Environment probe failed."),
        tone: "error",
      });
    },
  });
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok ? t("Draft probe passed") : t("Draft probe failed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("Select a company to manage environments.")}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("Company Environments")}</h1>
        </div>
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      ...
      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("Environment choices use the same adapter support matrix as agent defaults. SSH is always available for remote-managed adapters, and sandbox environments appear only when a run-capable sandbox provider plugin is installed.")}
        </div>
        {sandboxCreationEnabled ? (
          <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            {t("Installed sandbox providers:")}{" "}
            <span className="font-medium text-foreground">
              {discoveredPluginSandboxProviders.map((provider) => provider.displayName).join(", ")}
            </span>
            . {t("These are not adapter types. They back the Sandbox driver for adapters that support sandbox execution.")}
          </div>
        ) : null}

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("Environment support by adapter")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("Adapter")}</th>
                <th className="px-3 py-2 font-medium">{t("Local")}</th>
                <th className="px-3 py-2 font-medium">{t("SSH")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("Sandbox via plugin")}</th>
                ) : null}
              </tr>
            </thead>
              ...
              {(environmentCapabilities?.adapters.map((support) => ({
                ...
                <tr key={adapterType}>
                  <td className="py-2 pr-3 font-medium">
                    {t(adapterLabels[adapterType] ?? adapterType)}
                  </td>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("No environments saved for this company yet.")}</div>
          ) : (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      <div className="text-sm font-medium">
                        {environment.name} <span className="text-muted-foreground">· {driverLabel(environment.driver, t)}</span>
                      </div>
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string" ? environment.config.host : t("SSH host")} ·{" "}
                          {typeof environment.config.username === "string" ? environment.config.username : t("user")}
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {(() => {
                            ...
                            return t("{{name}} sandbox provider{{summary}}", {
                              name: displayName,
                        ...
                        <div className="text-xs text-muted-foreground">{t("Runs on this Paperclip host.")}</div>
                      )}
                    </div>
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId ? t("Edit environment") : t("Add environment")}
          </div>
          <div className="space-y-3">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="Private key" hint="Optional PEM private key. Leave blank to rely on the server's SSH agent or default keychain.">
                  <div className="space-y-2">
                    ...
                    >
                      <option value="">{t("No saved secret")}</option>
                      {(secrets ?? []).map((secret) => (
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("This provider does not declare additional configuration fields.")}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending ? t("Testing...") : t("Test draft")}
                </Button>
              ) : null}
