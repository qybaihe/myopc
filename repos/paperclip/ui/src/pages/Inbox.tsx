...
export function formatJoinRequestInboxLabel(
  ...
  t: (key: string, params?: Record<string, string | number | boolean | null | undefined>) => string =
    (key, params) => {
      ...
      return key.replace(/\{\{\s*([^}]+?)\s*\}\}/g, (_match, name: string) => {
          ...
          <span className="min-w-0 flex-1">
            <span className="line-clamp-2 text-sm font-medium sm:truncate sm:line-clamp-none">
              {issue ? (
                ...
                <>{linkedAgentName
                  ? t("Failed run — {{name}}", { name: linkedAgentName })
                  : t("Failed run")}</>
              )}
            </span>
      ...
      <div className="flex items-start gap-2 sm:items-center">
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            ...
            <span className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
              <span className="capitalize">{t(approval.status.replaceAll("_", " "))}</span>
                        ...
                        <DialogDescription>
                          {t(
                            unreadIssueIds.length === 1
                              ? "This will mark {{count}} unread item as read."
                              : "This will mark {{count}} unread items as read.",
                    ...
                    <IssueRow
                      ...
                      titleSuffix={hasChildren && !isExpanded && depth === 0 ? (
                        <span className="ml-1.5 text-xs text-muted-foreground">
                          {childCount === 1
                            ? t("({{count}} sub-task)", { count: childCount })
                            : t("({{count}} sub-tasks)", { count: childCount })}
                        </span>
                      ) : undefined}
                        ...
                        <IssueGroupHeader
                          ...
                          trailing={canCreateIssueInGroup ? (
                            <Button
                              ...
                              title={t("New issue in {{group}}", { group: t(group.label) })}
                              aria-label={t("New issue in {{group}}", { group: t(group.label) })}
                              onClick={(event) => {
                                event.stopPropagation();
