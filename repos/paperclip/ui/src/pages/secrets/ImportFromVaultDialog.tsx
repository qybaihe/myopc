...
function statusBadgeLabel(status: RemoteSecretImportCandidate["status"], t: TranslateFn = passthroughT) {
  switch (status) {
    case "duplicate":
      return t("Imported");
    case "conflict":
  ...
  switch (status) {
    case "imported":
      return (
        ...
        >
          <CheckCircle2 className="h-3 w-3" /> {t("Created")}
        </Badge>
      );
    case "skipped":
      return (
        ...
        >
          <Link2 className="h-3 w-3" /> {t("Skipped")}
        </Badge>
      );
    case "error":
    default:
...
function formatRelativeShort(value: string | null | undefined): string {
  ...
  if (diff < 0) return date.toLocaleDateString(locale);
...
function readableErrorMessage(error: unknown, t: TranslateFn = passthroughT): string {
  if (error instanceof ApiError) {
    return error.message || t("Request failed: {{status}}", { status: error.status });
  }
...
function validateDraftRow(
  ...
  t: TranslateFn = passthroughT,
): string | null {
  if (!draft.name.trim()) return t("Name is required.");
  if (draft.name.length > 160) return t("Name must be 160 characters or fewer.");
  if (!draft.key.trim()) return t("Key is required.");
  if (!KEY_PATTERN.test(draft.key)) {
    return t("Key may only contain lowercase letters, numbers, dot, underscore, or hyphen.");
  }
  if (draft.key.length > 120) return t("Key must be 120 characters or fewer.");
  if (draft.description.length > 500) return t("Description must be 500 characters or fewer.");
  ...
  for (const existingSecret of existing) {
    if (existingSecret.name.trim().toLowerCase() === lowerName) {
      return t("A Paperclip secret already uses this name.");
    }
    if (existingSecret.key.trim().toLowerCase() === lowerKey) {
      return t("A Paperclip secret already uses this key.");
    }
  }
  ...
  for (const other of otherDrafts) {
    ...
    if (other.name.trim().toLowerCase() === lowerName) {
      return t("Another row in this batch already uses this name.");
    }
    if (other.key.trim().toLowerCase() === lowerKey) {
      return t("Another row in this batch already uses this key.");
    }
  }
    ...
    onSuccess: (result) => {
      ...
      if (result.errorCount === draftList.length && result.errorCount > 0) {
        toast.pushToast({
          ...
          body: t("No secrets were imported from {{vaultName}}.", { vaultName }),
          tone: "error",
        });
      } else {
        toast.pushToast({
          title: result.errorCount > 0 ? t("Import completed with errors") : t("Import complete"),
          body: t("{{imported}} created · {{skipped}} skipped · {{failed}} failed", {
  ...
  function handleClose(force = false) {
    ...
    if (!force && step !== "result" && selection.size > 0 && !importResult) {
      const ok = window.confirm(
        t("Discard {{count}} pending import(s)?", { count: selection.size }),
      );
      if (!ok) return;
    }
    onOpenChange(false);
        ...
        <header className="flex items-start justify-between gap-3 border-b border-border/60 px-5 py-4">
          <div className="flex flex-col gap-1">
            <DialogTitle className="text-base font-semibold">
              {t("Import from AWS Secrets Manager")}
            </DialogTitle>
            <DialogDescription className="text-xs text-muted-foreground">
              {t("Bring AWS-managed secrets into Paperclip as external references.")}
            </DialogDescription>
            <Stepper step={step} />
          </div>
          <button
          ...
          <div className="flex items-center gap-2">
            ...
            {step === "review" && (
              ...
              >
                {importMutation.isPending ? (
                  <>
                    <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("Importing…")}
                  </>
                ) : (
                  t("Import {{count}}", { count: draftList.length })
                )}
              </Button>
            )}
            {step === "result" && (
  ...
  if (noEligibleVaults) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6" data-testid="select-empty-vaults">
        <EmptyState
          icon={Cloud}
          message={t("No AWS provider vault configured. Add one to import secrets.")}
          action={onManageVaults ? t("Manage vaults") : undefined}
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-border/60 px-5 py-3">
        <label className="text-xs uppercase tracking-wide text-muted-foreground">{t("Vault")}</label>
        {awsVaults.length === 1 && eligible.length === 1 ? (
          ...
          >
            <SelectTrigger size="sm" className="text-xs" aria-label={t("Select AWS vault")}>
              <SelectValue placeholder={t("Select an AWS vault")} />
            </SelectTrigger>
            <SelectContent>
              {awsVaults.map((vault) => {
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      {vault.isDefault && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px]">{t("default")}</Badge>
                      )}
                      {vault.status === "warning" && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px] text-amber-500 border-amber-500/40">{t("warning")}</Badge>
                      )}
                      {blocked && (
                        <Badge variant="outline" className="px-1 py-0 text-[10px] text-muted-foreground">
                          {vault.status === "coming_soon" ? t("coming soon") : t(vault.status)}
                        </Badge>
                      )}
                    </span>
                  </SelectItem>
        ...
        <div className="relative ml-auto w-64">
          ...
          <Input
            ...
            placeholder={t("Search by name, ARN, tag")}
            className="pl-7 pr-7 text-xs"
            aria-label={t("Search remote secrets")}
        ...
        <Button
          ...
          aria-label={t("Refresh remote secrets")}
        >
      ...
      {selectedNotVisible > 0 && (
        <div className="flex items-center justify-between border-b border-border/60 bg-muted/20 px-5 py-1.5 text-xs text-muted-foreground">
          <span>
            {t("{{selected}} selected · {{hidden}} not visible with current search", {
          ...
          >
            {showOnlySelected ? t("Show all") : t("Show selected")}
          </Button>
        </div>
      )}
      ...
      <div className="min-h-0 flex-1 overflow-y-auto" data-testid="vault-table-scroll">
              ...
              <tr>
                <th className="px-3 py-2 text-left">
                  <Checkbox
                    ...
                    aria-label={t("Select all loaded ({{count}})", { count: selectableInLoaded.length })}
                    disabled={selectableInLoaded.length === 0}
                  />
                </th>
                <th className="px-2 py-2 text-left font-medium">{t("Remote name")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("Reference")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("Last changed")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("Suggested name")}</th>
                <th className="px-2 py-2 text-left font-medium">{t("State")}</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        ...
                        aria-label={t("Select {{name}}", { name: candidate.remoteName })}
                      />
                    </td>
        ...
        {hasNextPage && !previewError && (
          <div className="flex items-center justify-between border-t border-border/60 px-5 py-2 text-xs text-muted-foreground">
            <span>
              {t("{{count}} loaded", { count: candidates.length })}
              {selectableInLoaded.length > 0 && (
                <span>{t(" · {{count}} selectable", { count: selectableInLoaded.length })}</span>
              )}
            </span>
            <Button
              variant="outline"
            ...
            >
              {pageLoading ? (
                <>
                  <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> {t("Loading…")}
                </>
              ) : (
                t("Load {{count}} more", { count: PAGE_SIZE })
              )}
            </Button>
          </div>
        )}
    ...
    >
      ...
      <div className="flex-1">
        <div className="font-medium">
          {isPermission
            ? t("AWS denied list access")
            : isThrottling
              ? t("AWS throttled the listing request")
              : t("Could not load remote secrets")}
        </div>
        <div className="mt-1 text-xs leading-relaxed text-destructive/80">
          {isPermission
            ? t("The AWS principal behind this vault is missing secretsmanager:ListSecrets. Update IAM and try again.")
            : message}
        </div>
        <div className="mt-2 flex items-center gap-2">
          ...
          {isPermission && (
            ...
            >
              {t("IAM reference")} <ExternalLink className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
...
function EmptyCandidates({ query }: { query: string }) {
  ...
  if (query) {
    return (
      <EmptyState
        icon={Search}
        message={t("No remote secrets match \"{{query}}\".", { query })}
      />
    );
  }
  return (
    <EmptyState
      icon={Database}
      message={t("No secrets visible to this vault.")}
    />
  );
}
  ...
  if (drafts.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 items-center justify-center p-6">
        <EmptyState
          icon={Info}
          message={t("No secrets selected. Go back to pick remote secrets to import.")}
        />
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-3 border-b border-border/60 bg-muted/20 px-5 py-3 text-xs">
        <span className="font-medium">{t("{{count}} secrets ready to import", { count: ready })}</span>
        {blocked > 0 && (
          <span className="text-amber-600 dark:text-amber-400">
            {t("{{count}} need attention before import", { count: blocked })}
          </span>
        )}
      </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1 space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("Paperclip name")}</span>
                      <Input
                        value={draft.name}
                        onChange={(e) =>
                    ...
                    <label className="flex flex-col gap-1 text-xs">
                      <span className="text-muted-foreground">{t("Description (optional)")}</span>
                      <Input
                ...
                <Button
                  ...
                  aria-label={t("Remove {{name}}", { name: draft.candidate.remoteName })}
                  className="h-7 w-7"
                  disabled={importing}
                >
  ...
  const heading =
    result.errorCount === result.results.length && result.errorCount > 0
      ...
      : result.errorCount === 0 && result.skippedCount === 0
        ? t("All {{count}} secrets imported", { count: result.importedCount })
        : t("Import complete");
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-border/60 px-5 py-3" data-testid="result-summary">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
          <span className="text-emerald-600 dark:text-emerald-400">✓ {t("{{count}} created", { count: result.importedCount })}</span>
          <span>⊘ {t("{{count}} skipped", { count: result.skippedCount })}</span>
          <span className="text-destructive">⨯ {t("{{count}} failed", { count: result.errorCount })}</span>
        </div>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {grouped.created.length > 0 && (
          <ResultGroup label={t("Created")} rows={grouped.created} draftLookup={draftLookup} />
        )}
        {grouped.skipped.length > 0 && (
          <ResultGroup label={t("Skipped")} rows={grouped.skipped} draftLookup={draftLookup} />
        )}
        {grouped.failed.length > 0 && (
          <ResultGroup label={t("Failed")} rows={grouped.failed} draftLookup={draftLookup} />
        )}
      </div>
    </div>
  );
...
function FooterStatus({
  ...
  if (step === "select") {
    return (
      <div className="text-xs text-muted-foreground">
        {totalSelected === 0
          ? t("Select remote secrets to import")
  ...
  if (step === "review") {
    return (
      <div className="text-xs text-muted-foreground">
        {t("{{count}} ready", { count: readyReviewCount })}
        {blockedReviewCount > 0 && (
          <span className="ml-2 text-amber-600 dark:text-amber-400">
            {t("· {{count}} blocked", { count: blockedReviewCount })}
          </span>
        )}
      </div>
    );
  ...
  if (result) {
    return (
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        <span>{t("{{count}} created", { count: result.importedCount })}</span>
        <span>{t("{{count}} skipped", { count: result.skippedCount })}</span>
        <span>{t("{{count}} failed", { count: result.errorCount })}</span>
      </div>
    );
  }
  return null;
