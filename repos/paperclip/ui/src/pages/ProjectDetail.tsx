...
import { ProjectOpenCodePanel } from "../components/ProjectOpenCodePanel";
...
type ProjectBaseTab = "overview" | "list" | "opencode" | "plugin-operations" | "workspaces" | "configuration" | "budget";
...
function resolveProjectTab(pathname: string, projectId: string): ProjectTab | null {
  ...
  if (tab === "opencode") return "opencode";
...
export function ProjectDetail() {
  ...
  useEffect(() => {
    ...
    if (activeTab === "opencode") {
      navigate(`/projects/${canonicalProjectRef}/opencode`, { replace: true });
      return;
    }
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (cachedTab === "opencode") {
      return <Navigate to={`/projects/${canonicalProjectRef}/opencode`} replace />;
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (tab === "overview") {
      ...
      navigate(`/projects/${canonicalProjectRef}/opencode`);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              {t("Managed by {{plugin}}", { plugin: project.managedByPlugin.pluginDisplayName })}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            ...
            { value: "opencode", label: t("OpenCode") },
      ...
      {activeTab === "opencode" && resolvedCompanyId ? (
        <ProjectOpenCodePanel
          project={project}
          companyId={resolvedCompanyId}
          projectRef={canonicalProjectRef}
          onWorkspaceSaved={invalidateProject}
        />
      ) : null}
