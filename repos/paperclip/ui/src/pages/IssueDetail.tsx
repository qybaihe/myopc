...
function issueTreeControlLabel(t: TranslateFn, mode: IssueTreeControlMode, scope: "leaf" | "subtree") {
  return scope === "leaf"
    ? t(LEAF_WORK_CONTROL_MODE_LABEL[mode] ?? TREE_CONTROL_MODE_LABEL[mode])
    : t(TREE_CONTROL_MODE_LABEL[mode]);
}

function issueTreeControlHelpText(t: TranslateFn, mode: IssueTreeControlMode, scope: "leaf" | "subtree") {
  return scope === "leaf"
    ? t(LEAF_WORK_CONTROL_MODE_HELP_TEXT[mode] ?? TREE_CONTROL_MODE_HELP_TEXT[mode])
    : t(TREE_CONTROL_MODE_HELP_TEXT[mode]);
}

function treeControlPreviewErrorCopy(t: TranslateFn, error: unknown): string {
              ...
              <div className="flex flex-wrap gap-3">
                ...
                {issueCostSummary.hasRuntime ? (
                  <span>
                    ...
                    {" "}
                    {t(
                      issueCostSummary.runCount === 1 ? "({{count}} run)" : "({{count}} runs)",
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {issueTreeCostSummary.runCount > 0 ? (
                    <span>
                      ...
                      {" "}
                      {t(
                        issueTreeCostSummary.runCount === 1 ? "({{count}} run)" : "({{count}} runs)",
                  ...
                  <span>
                    {t(
                      issueTreeCostSummary.issueCount === 1 ? "{{count}} issue" : "{{count}} issues",
...
export function IssueDetail() {
      ...
      pushToast({
        title: result.kind === "release"
          ...
          : result.hold.mode === "pause"
            ...
            : t("{{label}} applied", { label: modeLabel }),
        body: result.kind === "release"
          ...
          : result.hold.mode === "pause"
            ? treeControlScope === "leaf"
              ? t(
                cancelCount === 1
                  ? "Work paused. {{count}} run cancelled."
                  : "Work paused. {{count}} runs cancelled.",
                { count: cancelCount },
              )
              : t(
                cancelCount === 1
                  ? "Subtree paused. {{count}} run cancelled."
                  : "Subtree paused. {{count}} runs cancelled.",
                { count: cancelCount },
              )
  ...
  const pauseIssueWorkRun = useMutation({
    mutationFn: async ({ runId, scope }: { runId: string; scope: "leaf" | "subtree" }) => {
      const created = await issuesApi.createTreeHold(issueId!, {
        mode: "pause",
        reason: t("Paused from active run controls."),
    ...
    onSuccess: (interaction) => {
      ...
      pushToast({
        title: interaction.kind === "request_confirmation"
          ...
          ? t(
            createdCount === 1
              ? "Accepted {{created}} draft and skipped {{skipped}}"
              : "Accepted {{created}} drafts and skipped {{skipped}}",
  ...
  const treeControlPrimaryButtonLabel =
    treeControlMode === "pause"
      ...
      : treeControlMode === "cancel"
        ? t("Cancel {{count}} issues", { count: previewAffectedIssueCount })
      : treeControlMode === "restore"
          ? t("Restore {{count}} issues", { count: previewAffectedIssueCount })
          : treeControlScope === "leaf"
            ? t("Resume work")
            : t("Resume subtree");
            ...
            <div className="space-y-2">
              ...
              <div className="text-xs text-amber-900/80 dark:text-amber-100/80">
                {childIssues.length === 0
                  ? t("1 issue held")
                  : t(
                    heldDescendantCount === 1 ? "{{count}} descendant held" : "{{count}} descendants held",
              ...
              {canShowSubtreeControls || canResumeLeafWork ? (
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {t("View affected ({{count}})", { count: childIssues.length === 0 ? 1 : heldDescendantCount })}
                  </Button>
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-6 py-4">
            ...
            {(treeControlMode === "resume" || treeControlMode === "restore") ? (
              <div className="space-y-2">
                <label className="flex items-start gap-2 text-sm">
                  ...
                  />
                  <span>
                    <span className="block font-medium">{t("Wake affected agents ({{count}})", { count: previewAffectedAgentCount })}</span>
            ...
            {treeControlMode === "cancel" ? (
              <label className="flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/5 p-2 text-sm">
                ...
                />
                <span>{t("I understand this will cancel {{count}} issues.", { count: previewAffectedIssueCount })}</span>
              </label>
            ) : null}
