  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        throw new Error(
          failures.length === 1
            ? `${t("Failed to disable all heartbeats.")} ${detail}`
            : `${t("Failed to disable all heartbeats.")} ${detail}`,
        );
      }
      return enabled;
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t(grouped.length === 1 ? "company" : "companies")}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("agent") : t("agents");
              if (!window.confirm(t("Disable timer heartbeats for all {{count}} enabled {{noun}}?", { count: enabledCount, noun }))) {
                return;
              }
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          ...
                          >
                            {saving ? t("Working...") : agent.heartbeatEnabled ? t("Disable Timer Heartbeat") : t("Enable Timer Heartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
