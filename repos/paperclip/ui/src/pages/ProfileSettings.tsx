...
export function ProfileSettings() {
  ...
  function resolveProfileName() {
    return name.trim() || sessionQuery.data?.user.name || t("Board");
  }
  ...
  const uploadAvatarMutation = useMutation({
    mutationFn: async (file: File) => {
      if (!selectedCompanyId) {
        throw new Error(t("Select a company to upload an avatar into Paperclip storage."));
      }
  ...
  const currentName = name.trim() || sessionQuery.data.user.name || t("Board");
  ...
  const uploadHint = selectedCompany
    ? t("Stored in Paperclip file storage for {{company}}.", { company: selectedCompany.name })
