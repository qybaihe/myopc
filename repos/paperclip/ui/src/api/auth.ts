...
export const authApi = {
  ...
  signInEmail: async (input: { email: string; password: string }) =>
    authPost<EmailAuthResult>("/sign-in/email", input),

  signUpEmail: async (input: { name: string; email: string; password: string }) =>
    authPost<EmailAuthResult>("/sign-up/email", input),

  sendVerificationOtp: async (input: { email: string; type: EmailOtpType }) =>
    authPost<{ success: boolean }>("/email-otp/send-verification-otp", input),

  verifyEmailOtp: async (input: { email: string; otp: string }) =>
    authPost<EmailAuthResult & { status: boolean }>("/email-otp/verify-email", input),
...
export function isEmailNotVerifiedError(error: unknown) {
  if (error instanceof AuthApiError) {
    return error.message.trim().toLowerCase() === "email not verified";
  }
  return error instanceof Error && error.message.trim().toLowerCase() === "email not verified";
}
