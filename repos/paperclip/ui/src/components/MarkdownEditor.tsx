...
function mentionMarkdownLink(option: MentionOption): string {
...
function mentionMarkdown(option: MentionOption): string {
  return `${mentionMarkdownLink(option)} `;
}
...
function isAsciiIdentifierChar(value: string | undefined) {
  return Boolean(value && /^[A-Za-z0-9_-]$/.test(value));
}

function isPlainMentionStartBoundary(value: string, atIndex: number) {
  const previous = value[atIndex - 1];
  return !previous || /[\s([{<"'“‘，。！？、；：]/u.test(previous);
}

function isPlainMentionEndBoundary(name: string, next: string | undefined) {
  if (!next) return true;
  if (/[\s,.;:!?，。！？、；：)\]}>"'”’]/u.test(next)) return true;
  return hasCjkText(name) && !isAsciiIdentifierChar(next);
}

function isExistingMarkdownMention(value: string, atIndex: number, nameLength: number) {
  return value[atIndex - 1] === "[" && value.slice(atIndex + 1 + nameLength, atIndex + 3 + nameLength) === "](";
}

export function normalizePlainTextMentionMarkdown(value: string, mentions: readonly MentionOption[] = []): string {
  if (!value || mentions.length === 0 || !value.includes("@")) return value;

  const uniqueMentionByName = new Map<string, MentionOption | null>();
  for (const mention of mentions) {
    const normalizedName = mention.name.trim().toLowerCase();
    if (!normalizedName) continue;
    uniqueMentionByName.set(
      normalizedName,
      uniqueMentionByName.has(normalizedName) ? null : mention,
    );
  }

  const candidates = [...uniqueMentionByName.entries()]
    .filter((entry): entry is [string, MentionOption] => entry[1] !== null)
    .sort((left, right) => right[0].length - left[0].length);
  if (candidates.length === 0) return value;

  let normalized = "";
  for (let index = 0; index < value.length;) {
    if (value[index] !== "@" || !isPlainMentionStartBoundary(value, index)) {
      normalized += value[index] ?? "";
      index += 1;
      continue;
    }

    let matched: { name: string; option: MentionOption } | null = null;
    for (const [name, option] of candidates) {
      if (value.slice(index + 1, index + 1 + name.length).toLowerCase() !== name) continue;
      if (isExistingMarkdownMention(value, index, name.length)) continue;
      if (!isPlainMentionEndBoundary(name, value[index + 1 + name.length])) continue;
      matched = { name, option };
      break;
    }
    ...
    normalized += mentionMarkdownLink(matched.option);
