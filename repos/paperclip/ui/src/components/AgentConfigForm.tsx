...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const {
    ...
    queryFn: () => {
      if (!selectedCompanyId) {
        throw new Error(t("Select a company to detect the model"));
      }
  ...
  async function handleRefreshModels() {
    ...
    try {
      ...
      setRefreshModelsError(error instanceof Error ? error.message : t("Failed to refresh adapter models."));
    } finally {
    ...
    <div className={cn("relative", cards && "space-y-6")}>
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("Prompt template is replayed on every heartbeat. Keep it compact and dynamic to avoid recurring token cost and cache churn.")}
                </div>
              </>
            )}
          </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("Execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("Execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
            ...
            >
              ...
              >
                <option value="">{t("Company default (Local)")}</option>
        ...
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          ...
          {showInlineAdapterTestEnvironmentButton && (
            <Button
              ...
              disabled={testEnvironmentDisabled}
            >
              {testEnvironment.isPending ? t("Testing...") : t("Test")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              ...
              {supportsModelProfiles && (
                <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("Primary model")}</div>
              )}
              <ModelDropdown
                models={models}
              ...
              {adapterType === "opencode_local"
                ...
                <p className="text-xs text-muted-foreground">
                  {t("Live OpenCode model discovery only runs for Local environments. Using the curated list and manual entry for {{name}}.", { name: currentDefaultEnvironment.name })}
                </p>
              )}
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("Bootstrap prompt is legacy and will be removed in a future release. Consider moving this content into the agent's prompt template or instructions file instead.")}
                  </div>
                </>
              )}
      ...
      <PopoverTrigger asChild>
        <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
          <span className="inline-flex min-w-0 items-center gap-1.5">
            ...
            <span className="truncate">{t(adapterLabels[value] ?? getAdapterLabel(value))}</span>
          ...
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value
                  || (allowDefault ? t(defaultLabel ?? "Default") : required ? t("Select model (required)") : t("Select model"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("Search models... (type to create)") : t("Search models...")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel
                ? t("Detecting...")
                : detectedModel
                  ? t(detectModelLabel?.replace(/^Detect\b/, "Re-detect") ?? "Re-detect from config")
                  : t(detectModelLabel ?? "Detect from config")}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("Refreshing...") : t("Refresh models")}
            </button>
          )}
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? t(emptyDetectHint ?? "No model detected yet. Enter a provider/model manually.")
                    : t("No models found.")}
                </p>
              </div>
            )}
...
function CheapModelSection({
  ...
  const placeholderHint = adapterDefaultModel
    ? t("Adapter default · {{model}}", { model: adapterDefaultModel })
    : t("No adapter default — choose a cheaper model");
  return (
    <div className="rounded-md border border-border/70 bg-muted/20 p-3 space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("Cheap model")}</div>
          <p className="text-xs text-muted-foreground">
            {t("Used when a run requests the cheap profile (e.g. routine summaries). The primary model stays unchanged.")}
          </p>
        </div>
      ...
      {enabled && !model && adapterDefaultModel ? (
        <p className="text-[11px] text-muted-foreground">
          {t("No explicit cheap model selected — runtime falls back to")} <code>{adapterDefaultModel}</code>.
        </p>
      ) : null}
      {enabled && !model && !adapterDefaultModel ? (
        <p className="text-[11px] text-amber-500">
          {t("No cheap model selected and the adapter has no default. Cheap-lane runs will continue on the primary model with a fallback note.")}
        </p>
      ) : null}
    </div>
  );
  ...
  return (
    <Field label="Thinking effort" hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{t(selected?.label ?? "Auto")}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
