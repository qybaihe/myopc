...
import { browserVisibleProjectServiceUrl, cn, formatDate } from "../lib/utils";
            ...
            {primaryCodebaseWorkspace?.runtimeServices && primaryCodebaseWorkspace.runtimeServices.length > 0 ? (
              <div className="space-y-1">
                {primaryCodebaseWorkspace.runtimeServices.map((service) => {
                  const visibleServiceUrl = browserVisibleProjectServiceUrl(service.url, project);
                  return (
                    <div
                      key={service.id}
