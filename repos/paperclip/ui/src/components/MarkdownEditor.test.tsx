...
import {
  ...
  MarkdownEditor,
  normalizePlainTextMentionMarkdown,
...
describe("MarkdownEditor", () => {
  ...
  it("normalizes plain-text agent mentions before submit", () => {
    expect(normalizePlainTextMentionMarkdown("@深度分析员：帮我分析一下", [
      { id: "agent:agent-1", name: "深度分析员", kind: "agent", agentId: "agent-1", agentIcon: "brain" },
    ])).toBe(`[@深度分析员](${buildAgentMentionHref("agent-1", "brain")})：帮我分析一下`);

    expect(normalizePlainTextMentionMarkdown("cc @Deep Analyst: review this", [
      { id: "agent:agent-2", name: "Deep Analyst", kind: "agent", agentId: "agent-2", agentIcon: null },
    ])).toBe(`cc [@Deep Analyst](${buildAgentMentionHref("agent-2", null)}): review this`);
  });

  it("leaves existing and ambiguous mention text unchanged", () => {
    const existing = `[@Deep Analyst](${buildAgentMentionHref("agent-2", null)}) please review`;

    expect(normalizePlainTextMentionMarkdown(existing, [
      { id: "agent:agent-2", name: "Deep Analyst", kind: "agent", agentId: "agent-2", agentIcon: null },
    ])).toBe(existing);

    expect(normalizePlainTextMentionMarkdown("@Deep Analyst please review", [
      { id: "agent:agent-2", name: "Deep Analyst", kind: "agent", agentId: "agent-2", agentIcon: null },
      { id: "agent:agent-3", name: "deep analyst", kind: "agent", agentId: "agent-3", agentIcon: null },
    ])).toBe("@Deep Analyst please review");
  });
