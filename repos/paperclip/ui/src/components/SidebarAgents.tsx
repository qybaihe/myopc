...
function SidebarAgentItem({
  ...
  const pauseResumeLabel = isPaused ? t("Resume agent") : t("Pause agent");
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            aria-label={t("Open actions for {{name}}", { name: agent.name })}
          >
        ...
        <DropdownMenuContent align="end" className="w-44">
          ...
          <DropdownMenuItem
            ...
            title={isBudgetPaused ? t("Agent was paused by budget limits") : undefined}
          >
...
export function SidebarAgents() {
  ...
  const pauseResumeAgent = useMutation({
    ...
    onSuccess: async (_agent, { agent, action }) => {
      ...
      pushToast({
        title: action === "pause" ? t("Agent paused") : t("Agent resumed"),
        body: agent.name,
        tone: "success",
      });
    },
    onError: (error, { agent, action }) => {
      pushToast({
        title: action === "pause" ? t("Could not pause agent") : t("Could not resume agent"),
  ...
  return (
    <SidebarSection
      ...
      headerAction={{
        ariaLabel: t("New agent"),
        icon: Plus,
        onClick: openNewAgent,
      }}
      menu={{
        ariaLabel: t("Agents section actions"),
        actions: [
          { type: "item", label: t("Browse agents"), icon: Users, href: "/agents/all" },
          { type: "separator" },
        ],
        radioLabel: t("Agent sort"),
