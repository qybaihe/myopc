...
import { knowledgeService, projectService, logActivity, workspaceOperationService } from "../services/index.js";
...
export function projectRoutes(db: Db) {
  ...
  router.get("/projects/:id/knowledge", async (req, res) => {
    ...
    assertCompanyAccess(req, project.companyId);
    res.json(await knowledge.list({ scopeType: "project", companyId: project.companyId, projectId: project.id }));
  });

  router.post("/projects/:id/knowledge", validate(createKnowledgeDocumentSchema), async (req, res) => {
    const id = req.params.id as string;
    const project = await svc.getById(id);
    if (!project) {
      res.status(404).json({ error: "Project not found" });
    ...
    const created = await knowledge.create(
      { scopeType: "project", companyId: project.companyId, projectId: project.id },
      req.body,
    );
    res.status(201).json(created);
  });

  router.get("/projects/:id/knowledge/:docId", async (req, res) => {
    const id = req.params.id as string;
    const project = await svc.getById(id);
    if (!project) {
      res.status(404).json({ error: "Project not found" });
  ...
  router.patch("/projects/:id/knowledge/:docId", validate(updateKnowledgeDocumentSchema), async (req, res) => {
    const id = req.params.id as string;
    const project = await svc.getById(id);
    if (!project) {
      res.status(404).json({ error: "Project not found" });
