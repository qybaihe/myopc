...
import {
  ...
  patchInstanceAiGatewaySettingsSchema,
...
import {
  redactAiGatewayForLogs,
  redactInstanceGeneralSettingsForLogs,
  redactInstanceGeneralSettingsForRead,
...
export function instanceSettingsRoutes(db: Db) {
  ...
  router.get("/instance/settings/general", async (req, res) => {
    ...
    res.json(redactInstanceGeneralSettingsForRead(await svc.getGeneral()));
  });
      ...
      await Promise.all(
        companyIds.map((companyId) =>
          logActivity(db, {
            ...
            entityId: updated.id,
            details: {
              general: redactInstanceGeneralSettingsForLogs(updated.general),
              changedKeys: Object.keys(req.body).sort(),
            },
          }),
        ),
      );
      res.json(redactInstanceGeneralSettingsForRead(updated.general));
    },
  );

  router.get("/instance/settings/ai", async (req, res) => {
    assertCanManageInstanceSettings(req);
    res.json(await svc.getAiGateway());
  });

  router.patch(
    "/instance/settings/ai",
    validate(patchInstanceAiGatewaySettingsSchema),
    async (req, res) => {
      assertCanManageInstanceSettings(req);
      const updated = await svc.updateAiGateway(req.body);
      const actor = getActorInfo(req);
      const companyIds = await svc.listCompanyIds();
      await Promise.all(
        companyIds.map((companyId) =>
          logActivity(db, {
            ...
            agentId: actor.agentId,
            runId: actor.runId,
            action: "instance.settings.ai_updated",
            entityType: "instance_settings",
            entityId: updated.id,
            details: {
              aiGateway: redactAiGatewayForLogs(updated.general.aiGateway),
              changedKeys: Object.keys(req.body).sort(),
            },
          }),
        ),
      );
      res.json(updated.general.aiGateway);
    },
  );
