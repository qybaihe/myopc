...
type CompanyListItem = Awaited<ReturnType<ReturnType<typeof companyService>["list"]>>[number];

export function filterCompaniesForActor(
  rows: CompanyListItem[],
  actor: Request["actor"],
): CompanyListItem[] {
  if (actor.type !== "board") return [];
  if (actor.source === "local_implicit") return rows;
  const allowed = new Set(actor.companyIds ?? []);
  return rows.filter((company) => allowed.has(company.id));
}
...
export function companyRoutes(db: Db, storage?: StorageService) {
  ...
  const umami = umamiService();
  const uptimeKuma = uptimeKumaService();
  ...
  router.get("/", async (req, res) => {
    ...
    res.json(filterCompaniesForActor(result, req.actor));
  });
  ...
  router.get("/:companyId/knowledge", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    res.json(await knowledge.list({ scopeType: "company", companyId }));
  });

  router.get("/:companyId/products", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    res.json(await products.listCompanyProducts(companyId));
  });

  router.get("/:companyId/products/:productId/analytics", async (req, res) => {
    const companyId = req.params.companyId as string;
    const productId = req.params.productId as string;
    assertCompanyAccess(req, companyId);

    const catalog = await products.listCompanyProducts(companyId);
    const product = catalog.products.find((item) => item.id === productId || item.projectId === productId) ?? null;
    ...
    res.json(
      await umami.getDashboard({
        productName: product.name,
        siteUrl: product.site.url,
        createWebsiteIfMissing: true,
      }),
    );
  });

  router.get("/:companyId/monitoring/uptime-kuma", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    res.json(await uptimeKuma.getDashboard());
  });

  router.post("/:companyId/knowledge", validate(createKnowledgeDocumentSchema), async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
  ...
  router.get("/:companyId/knowledge/:docId", async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    try {
      res.json(await knowledge.get({ scopeType: "company", companyId }, req.params.docId as string));
    } catch (error) {
  ...
  router.patch("/:companyId/knowledge/:docId", validate(updateKnowledgeDocumentSchema), async (req, res) => {
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    try {
      res.json(
