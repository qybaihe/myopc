...
import { discoverOpenCodeModels, requireOpenCodeModelId } from "@paperclipai/adapter-opencode-local/server";
import {
...
import { listCodexModels, refreshCodexModels } from "../adapters/codex-models.js";
import { buildAiGatewayOpenAiEnv, mergeAiGatewayIntoAdapterConfig } from "../services/ai-gateway.js";
...
export function agentRoutes(
  ...
  router.get("/companies/:companyId/adapters/:type/models", async (req, res) => {
    ...
    const aiGateway = await instanceSettings.getAiGateway();
    if (type === "codex_local") {
      const models = refresh
        ? await refreshCodexModels({ aiGateway })
        : await listCodexModels({ aiGateway });
      res.json(models);
      return;
    }
    if (
      type === "opencode_local" &&
      aiGateway.provider === "openai_compatible" &&
      (!environment || environment.driver === "local")
    ) {
      try {
        const gatewayEnv = buildAiGatewayOpenAiEnv(aiGateway);
        if (Object.keys(gatewayEnv).length > 0) {
          const models = await discoverOpenCodeModels({ env: gatewayEnv });
          if (models.length > 0) {
            res.json(models);
            return;
          }
        ...
        // Fall back to the built-in discovery / static fallback path below.
      }
    }
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const aiGateway = await instanceSettings.getAiGateway();
      const { adapterConfig: effectiveRuntimeAdapterConfig } = mergeAiGatewayIntoAdapterConfig({
        adapterType: type,
