...
export function resolveKnowledgeRootDir(): string {
  return path.resolve(resolvePaperclipInstanceRoot(), "data", "knowledge");
}

export function resolveCompanyKnowledgeDir(companyId: string): string {
  const trimmed = companyId.trim();
  if (!PATH_SEGMENT_RE.test(trimmed)) {
    throw new Error(`Invalid company id for knowledge path '${companyId}'.`);
  }
  return path.resolve(resolveKnowledgeRootDir(), "companies", trimmed);
}

export function resolveProjectKnowledgeDir(projectId: string): string {
  const trimmed = projectId.trim();
  if (!PATH_SEGMENT_RE.test(trimmed)) {
    throw new Error(`Invalid project id for knowledge path '${projectId}'.`);
  }
  return path.resolve(resolveKnowledgeRootDir(), "projects", trimmed);
}
