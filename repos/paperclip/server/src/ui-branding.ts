...
const DEFAULT_MYOPC_OPENCODE_PUBLIC_URL = "https://code.myopc.me";
...
function readMyopcDefaultOpenCodePublicUrl(env: NodeJS.ProcessEnv): string | null {
  const raw =
    nonEmpty(env.PAPERCLIP_PUBLIC_URL)
    ?? nonEmpty(env.PAPERCLIP_AUTH_PUBLIC_BASE_URL)
    ?? nonEmpty(env.BETTER_AUTH_URL)
    ?? nonEmpty(env.BETTER_AUTH_BASE_URL);
  if (!raw) return null;
  try {
    const hostname = new URL(raw).hostname.toLowerCase();
    return hostname === "myopc.me" || hostname === "www.myopc.me" ? DEFAULT_MYOPC_OPENCODE_PUBLIC_URL : null;
  } catch {
    return null;
  }
}
...
export function renderRuntimeBrandingMeta(
  ...
  const opencodeWebUrl =
    readPublicUrl(env.PAPERCLIP_OPENCODE_PUBLIC_URL)
    ?? readPublicUrl(env.PAPERCLIP_MYOPC_CODE_ENGINE_PUBLIC_URL)
    ?? readMyopcDefaultOpenCodePublicUrl(env);
  if (opencodeWebUrl) {
    entries.push(
      `<script>window.__PAPERCLIP_RUNTIME_CONFIG__=${escapeHtmlScriptJson({ opencodeWebUrl })};</script>`,
    );
  }
