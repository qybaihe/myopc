...
describe("instance settings routes", () => {
  beforeEach(() => {
    ...
    mockInstanceSettingsService.getAiGateway.mockResolvedValue({
      enabled: true,
      provider: "openai_compatible",
      baseUrl: "https://gateway.example/v1",
      apiKey: "sk-test-secret",
    ...
    mockInstanceSettingsService.updateAiGateway.mockResolvedValue({
      id: "instance-settings-1",
      general: {
        censorUsernameInLogs: false,
        keyboardShortcuts: false,
  ...
  it("redacts AI credentials on the general settings read route", async () => {
    const app = await createApp({
      type: "board",
      userId: "user-1",
      source: "user_session",
    ...
    const res = await request(app).get("/api/instance/settings/general").expect(200);

    expect(res.body.aiGateway).toEqual({
      enabled: true,
      provider: "openai_compatible",
      baseUrl: "https://gateway.example/v1",
      apiKey: "",
  ...
  it("allows instance admins to read and update dedicated AI settings", async () => {
    const app = await createApp({
      type: "board",
      userId: "local-board",
      source: "local_implicit",
    ...
    const getRes = await request(app).get("/api/instance/settings/ai").expect(200);
    expect(getRes.body).toEqual({
      enabled: true,
      provider: "openai_compatible",
      baseUrl: "https://gateway.example/v1",
    ...
    const patchRes = await request(app)
      .patch("/api/instance/settings/ai")
      .send({
        enabled: true,
        baseUrl: "https://gateway.example/v1",
        apiKey: "sk-updated-secret",
    ...
    expect(mockInstanceSettingsService.updateAiGateway).toHaveBeenCalledWith({
      enabled: true,
      baseUrl: "https://gateway.example/v1",
      apiKey: "sk-updated-secret",
      defaultModel: "gpt-5.5",
    });
    expect(patchRes.body.apiKey).toBe("sk-updated-secret");
    expect(mockLogActivity).toHaveBeenCalledTimes(2);
    expect(mockLogActivity.mock.calls[0]?.[1]?.details?.aiGateway?.apiKey).toBe("***REDACTED***");
  });

  it("rejects dedicated AI settings reads for non-admin board users", async () => {
    const app = await createApp({
      type: "board",
      userId: "user-1",
      source: "user_session",
    ...
    await request(app).get("/api/instance/settings/ai").expect(403);
  });
