...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  it("resolves agent mentions from links and direct questions", async () => {
    const companyId = randomUUID();
    const analystId = randomUUID();
    const englishAnalystId = randomUUID();
    ...
    await expect(svc.findMentionedAgents(
      companyId,
      `[@深度分析员](${buildAgentMentionHref(analystId, "brain")}) 请分析`,
    )).resolves.toEqual([analystId]);
    await expect(svc.findMentionedAgents(companyId, "@深度分析员：帮我分析一下")).resolves.toEqual([analystId]);
    await expect(svc.findMentionedAgents(companyId, "@Deep Analyst: review this")).resolves.toEqual([englishAnalystId]);
    await expect(svc.findMentionedAgents(companyId, "email@example.com @深度分析员团队")).resolves.toEqual([]);
  });
});
