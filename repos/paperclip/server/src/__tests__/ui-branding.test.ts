...
describe("ui branding", () => {
  ...
  it("injects a public MyOPC Code Engine URL into the runtime config", () => {
    const meta = renderRuntimeBrandingMeta(getWorktreeUiBranding({}), {
      PAPERCLIP_OPENCODE_PUBLIC_URL: "https://code.myopc.me/path",
    });

    expect(meta).toContain("window.__PAPERCLIP_RUNTIME_CONFIG__");
    expect(meta).toContain('"opencodeWebUrl":"https://code.myopc.me"');
  });

  it("does not inject loopback MyOPC Code Engine URLs into the browser", () => {
    const meta = renderRuntimeBrandingMeta(getWorktreeUiBranding({}), {
      PAPERCLIP_OPENCODE_PUBLIC_URL: "http://127.0.0.1:4096",
    });

    expect(meta).not.toContain("window.__PAPERCLIP_RUNTIME_CONFIG__");
    expect(meta).not.toContain("127.0.0.1");
  });

  it("injects the default MyOPC Code Engine URL for hosted MyOPC deployments", () => {
    const meta = renderRuntimeBrandingMeta(getWorktreeUiBranding({}), {
