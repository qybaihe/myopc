...
export function shouldAutoCreateCompanyForNewUsers(deploymentMode: DeploymentMode): boolean {
  if (deploymentMode !== "authenticated") return false;

  const value = process.env.PAPERCLIP_AUTO_CREATE_COMPANY_ON_SIGNUP?.trim().toLowerCase();
...
export async function createApp(
  ...
  app.use(
    actorMiddleware(db, {
      ...
      autoCreateCompanyForNewUsers: shouldAutoCreateCompanyForNewUsers(opts.deploymentMode),
    }),
  );
