...
import { ensureMyopcDefaultAgentTeamsForMemberships } from "../services/default-company-agents.js";
...
interface ActorMiddlewareOptions {
  ...
  autoCreateCompanyForNewUsers?: boolean;
}

type BoardMembership = NonNullable<Express.Request["actor"]["memberships"]>[number];
      ...
      if (opts.deploymentMode === "authenticated" && opts.resolveSession) {
        ...
        if (session?.user?.id) {
          ...
          const effectiveMemberships =
            opts.autoCreateCompanyForNewUsers && memberships.length === 0 && !roleRow
              ? await ensureSelfServeCompanyForSessionUser(db, {
                  userId,
                  userName: session.user.name ?? null,
                  userEmail: session.user.email ?? null,
                })
              : memberships;
          if (opts.autoCreateCompanyForNewUsers && !roleRow) {
            await ensureMyopcDefaultAgentTeamsForMemberships(db, effectiveMemberships);
          }
          req.actor = {
            type: "board",
            userId,
...
async function ensureSelfServeCompanyForSessionUser(
  db: Db,
  input: {
    userId: string;
  ...
  const companyId = selfServeCompanyIdForUser(input.userId);
  ...
  return db.transaction(async (tx) => {
    ...
    if (existingMemberships.length > 0) return existingMemberships;

    await tx
      .insert(companies)
      .values({
        id: companyId,
        name: selfServeCompanyName(input.userName, input.userEmail),
        description: "Automatically created for self-serve registration.",
        status: "active",
        issuePrefix: issuePrefixForSelfServeUser(input.userId),
        updatedAt: now,
      })
      .onConflictDoNothing({
        target: companies.id,
    ...
    logger.info(
      { userId: input.userId, companyId },
      "Auto-created self-serve company for authenticated user",
    );
...
function cloudTenantCompanyId(stackId: string): string {
  return deterministicUuid("paperclip-cloud-tenant-company", stackId);
}

function selfServeCompanyIdForUser(userId: string): string {
  return deterministicUuid("paperclip-self-serve-company", userId);
}
...
function issuePrefixForSelfServeUser(userId: string): string {
  const hash = createHash("sha256").update(`paperclip-self-serve-issue-prefix:${userId}`).digest("hex");
  return `U${hash.slice(0, 10).toUpperCase()}`;
}

function selfServeCompanyName(userName: string | null, userEmail: string | null): string {
  const trimmedName = userName?.trim();
  if (trimmedName) return `${trimmedName}'s Company`;

  const emailName = userEmail?.split("@")[0]?.trim();
  if (emailName) return `${emailName}'s Company`;

  return "My Paperclip Company";
}
