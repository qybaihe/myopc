...
function isPlainAgentMentionStartBoundary(body: string, atIndex: number): boolean {
  const previous = body[atIndex - 1];
  return !previous || /[\s([{<"'“‘，。！？、；：]/u.test(previous);
}

function isPlainAgentMentionEndBoundary(next: string | undefined): boolean {
  return !next || /[\s,.;:!?，。！？、；：)\]}>"'”’]/u.test(next);
}

function isExistingAgentMentionMarkdownLink(body: string, atIndex: number, nameLength: number): boolean {
  return body[atIndex - 1] === "[" && body.slice(atIndex + 1 + nameLength, atIndex + 3 + nameLength) === "](";
}

function findAgentMentionIdsByName(
  body: string,
  agentRows: Array<{ id: string; name: string }>,
): string[] {
  if (!body.includes("@")) return [];

  const idsByNormalizedName = new Map<string, { name: string; ids: string[] }>();
  for (const agent of agentRows) {
    const name = normalizeAgentMentionToken(agent.name);
    if (!name) continue;
    const normalizedName = name.toLowerCase();
    const entry = idsByNormalizedName.get(normalizedName);
    if (entry) {
      entry.ids.push(agent.id);
    } else {
      idsByNormalizedName.set(normalizedName, { name, ids: [agent.id] });
    }
  }

  const candidates = [...idsByNormalizedName.entries()]
    .map(([normalizedName, entry]) => ({
      normalizedName,
      nameLength: entry.name.length,
      ids: entry.ids,
    }))
    .sort((left, right) => right.nameLength - left.nameLength);
  if (candidates.length === 0) return [];

  const resolved = new Set<string>();
  for (let index = 0; index < body.length; index += 1) {
    if (body[index] !== "@" || !isPlainAgentMentionStartBoundary(body, index)) continue;

    for (const candidate of candidates) {
      const start = index + 1;
      const end = start + candidate.nameLength;
      if (body.slice(start, end).toLowerCase() !== candidate.normalizedName) continue;
      if (isExistingAgentMentionMarkdownLink(body, index, candidate.nameLength)) continue;
      if (!isPlainAgentMentionEndBoundary(body[end])) continue;
      for (const id of candidate.ids) resolved.add(id);
      break;
    }
  }
...
export function issueService(db: Db) {
  ...
  return {
    ...
    findMentionedAgents: async (companyId: string, body: string) => {
      ...
      for (const agentId of findAgentMentionIdsByName(body, rows)) resolved.add(agentId);
      return [...resolved];
    },
