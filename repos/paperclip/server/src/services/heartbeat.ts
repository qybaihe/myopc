...
import { mergeAiGatewayIntoAdapterConfig } from "./ai-gateway.js";
import {
...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  async function executeRun(runId: string) {
    ...
    const aiGateway = (await instanceSettings.getGeneral()).aiGateway;
    const runScopedMentionedSkillKeys = await resolveRunScopedMentionedSkillKeys({
      db,
      companyId: agent.companyId,
      issueId,
    });
    const { adapterConfig: gatewayResolvedConfig, injectedSecretKeys } = mergeAiGatewayIntoAdapterConfig({
      adapterType: agent.adapterType,
    ...
    const sandboxContext = await resolveSandboxContext({
      companyId: agent.companyId,
      userId: readNonEmptyString(context.userId),
      projectId: executionWorkspace.projectId ?? issueRef?.projectId ?? executionProjectId ?? null,
      workspaceId: persistedExecutionWorkspace?.id ?? executionWorkspace.workspaceId ?? null,
      runId: run.id,
      // Phase 1 preserves the already-realized execution cwd so existing
      // project/worktree behavior keeps working while HOME/XDG/tmp/env are
      // isolated per tenant. A later migration can omit this and let the
      // resolver allocate the tenant workspace path directly.
    ...
    context.paperclipWorkspace = {
      ...
      agentHome: sandboxContext.homeDir,
    };
    context.paperclipSandbox = sandboxContext;
