...
const SENSITIVE_RUNTIME_INHERITED_ENV_KEYS = new Set([
  "DATABASE_URL",
  "RESEND_API_KEY",
  "BETTER_AUTH_SECRET",
...
const SENSITIVE_RUNTIME_INHERITED_ENV_KEY_RE = /(?:^|_)(?:API[_-]?KEY|SECRET|TOKEN|PASSWORD|PASSWD|AUTHORIZATION|COOKIE|PRIVATE[_-]?KEY|WEBHOOK)(?:$|_)/i;
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    ...
    if (
      SENSITIVE_RUNTIME_INHERITED_ENV_KEYS.has(key.toUpperCase()) ||
      SENSITIVE_RUNTIME_INHERITED_ENV_KEY_RE.test(key)
    ) {
      delete env[key];
    }
  }
